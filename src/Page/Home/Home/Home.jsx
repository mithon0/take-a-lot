import { Link } from "react-router-dom";
import { useEffect, useRef, useState } from "react";

import { Swiper, SwiperSlide } from 'swiper/react';

// Import Swiper styles
import 'swiper/css';
import 'swiper/css/pagination';
import 'swiper/css/navigation';
import "./home.css"
 
// import required modules
import { Pagination, Navigation } from 'swiper/modules';
import { HiStar } from "react-icons/hi2";
import { IoIosArrowDown } from "react-icons/io";

const Home = () => {
  const breakpoints = {
    768: {
      slidesPerView: 2,
      spaceBetween: 0 
    },
    1024: {
      slidesPerView: 4, 
      spaceBetween: 30
    },
  };
    const [allProducts , setAllProducts] = useState([]);

    useEffect(()=>{
        fetch(`http://localhost:5000/all-products?page=${1}&limit=${500}`)
        .then(res=>res.json())
        .then(data=>setAllProducts(data))
    },[])

    console.log(allProducts);


    return (
<section className="my-7 flex items-start w-full justify-center gap-10 px-5">
<div className="w-full lg:w-[72%] mt-7">
    <div className="product-container w-full">
<div className="w-full flex items-center justify-between mb-2">
<Link to="/brand/nivea" className="text-base lg:text-xl font-bold text-[#4d4d4f] hover:underline">NIVEA Premium Skin Care</Link>
<Link to="/all/nivea" className="px-3 py-1 border border-[#4d4d4f] text-[#4d4d4f]  font-semibold hover:bg-[#4d4d4f] hover:text-white transition-all duration-500"><button>View More</button></Link>
</div>

<div className="product-cards w-full my-10 h-full">
<Swiper
        // slidesPerView={1}
        breakpoints={breakpoints}
        spaceBetween={30}
        // loop={true}
        // pagination={{
        //   clickable: true,
        // }}
        navigation={true}
        modules={[Pagination, Navigation]}
        className="mySwiper"
      >
 
        {
allProducts.splice(0,10).map(prod=><SwiperSlide key={prod?._id} className="">

<Link to={`/product-details/${prod?.Name}/${prod?._id}`} className="w-[200px] h-[300px] flex flex-col gap-2 bg-white px-2 py-3 shadow hover:shadow-xl">

<div className="w-[150px] h-[120px] mx-auto">
<img src={prod?.Image_Url} alt={prod?.Name} />
</div>

<div className="h-[40px] mt-8">
  <p className="text-xs font-normal text-[#4d4d4f] overflow-hidden">{prod?.Name.slice(0,45)}{prod?.Name.length > 50 ? "..." : ""}</p>
</div>

<div className="mt-3 flex flex-col space-y-2">
<p className="font-bold">R 220</p>
<p className=" inline-flex items-center gap-1 text-sm"><HiStar className='h-4 w-4 text-yellow-400' /> <span>4.3</span><span className='font-medium text-gray-600'>(20)</span> <span><IoIosArrowDown className='h-5 w-5 text-gray-500' /></span> </p>
</div>

</Link>

</SwiperSlide>)
        }

      </Swiper>

</div>

    </div>

    <div className="product-container w-full my-5">
<div className="w-full flex justify-between items-center mb-2">
<Link to="/brand/apple" className="text-base lg:text-xl font-bold text-[#4d4d4f] hover:underline">Shop iPhone 15 and Apple Watch</Link>
<Link to="/all/nivea" className="px-3 py-1 border border-[#4d4d4f] text-[#4d4d4f]  font-semibold hover:bg-[#4d4d4f] hover:text-white transition-all duration-500"><button>View More</button></Link>
</div>

<div className="product-cards w-full my-10 h-full">
<Swiper
        // slidesPerView={1}
        breakpoints={breakpoints}
        spaceBetween={30}
        // loop={true}
        // pagination={{
        //   clickable: true,
        // }}
        navigation={true}
        modules={[Pagination, Navigation]}
        className="mySwiper"
      >
 
        {
allProducts.splice(11,20).map(prod=><SwiperSlide key={prod?._id} className="">

<Link to={`/product-details/${prod?.Name}/${prod?._id}`} className="w-[200px] h-[300px] flex flex-col gap-2 bg-white px-2 py-3 shadow hover:shadow-xl">

<div className="w-[150px] h-[120px] mx-auto">
<img src={prod?.Image_Url} alt={prod?.Name} />
</div>

<div className="h-[40px] mt-8">
  <p className="text-xs font-normal text-[#4d4d4f] overflow-hidden">{prod?.Name.slice(0,45)}{prod?.Name.length > 50 ? "..." : ""}</p>
</div>

<div className="mt-3 flex flex-col space-y-2">
<p className="font-bold">R 220</p>
<p className=" inline-flex items-center gap-1 text-sm"><HiStar className='h-4 w-4 text-yellow-400' /> <span>4.3</span><span className='font-medium text-gray-600'>(20)</span> <span><IoIosArrowDown className='h-5 w-5 text-gray-500' /></span> </p>
</div>

</Link>

</SwiperSlide>)
        }

      </Swiper>

</div>

    </div>

    <div className="product-container w-full my-5">
<div className="w-full flex justify-between items-center mb-2">
<Link to="/brand/avon" className=" text-base lg:text-xl font-bold text-[#4d4d4f] hover:underline">Go Far Away Beyond the Moon with Avon</Link>
<Link to="/all/nivea" className="px-3 py-1 border border-[#4d4d4f] text-[#4d4d4f]  font-semibold hover:bg-[#4d4d4f] hover:text-white transition-all duration-500"><button>View More</button></Link>
</div>

<div className="product-cards w-full my-10 h-full">
<Swiper
        // slidesPerView={1}
        breakpoints={breakpoints}
        spaceBetween={30}
        // loop={true}
        // pagination={{
        //   clickable: true,
        // }}
        navigation={true}
        modules={[Pagination, Navigation]}
        className="mySwiper"
      >
 
        {
allProducts.splice(20,30).map(prod=><SwiperSlide key={prod?._id} className="">

<Link to={`/product-details/${prod?.Name}/${prod?._id}`} className="w-[200px] h-[300px] flex flex-col gap-2 bg-white px-2 py-3 shadow hover:shadow-xl">

<div className="w-[150px] h-[120px] mx-auto">
<img src={prod?.Image_Url} alt={prod?.Name} />
</div>

<div className="h-[40px] mt-8">
  <p className="text-xs font-normal text-[#4d4d4f] overflow-hidden">{prod?.Name.slice(0,45)}{prod?.Name.length > 50 ? "..." : ""}</p>
</div>

<div className="mt-3 flex flex-col space-y-2">
<p className="font-bold">R 220</p>
<p className=" inline-flex items-center gap-1 text-sm"><HiStar className='h-4 w-4 text-yellow-400' /> <span>4.3</span><span className='font-medium text-gray-600'>(20)</span> <span><IoIosArrowDown className='h-5 w-5 text-gray-500' /></span> </p>
</div>

</Link>

</SwiperSlide>)
        }

      </Swiper>

</div>

    </div>


    <div className="product-container w-full my-5">
<div className="w-full flex justify-between items-center mb-2">
<Link to="/brand/oral-B" className="text-base lg:text-xl font-bold text-[#4d4d4f] hover:underline">Shop Oral-B Advanced Technology</Link>
<Link to="/all/nivea" className="px-3 py-1 border border-[#4d4d4f] text-[#4d4d4f]  font-semibold hover:bg-[#4d4d4f] hover:text-white transition-all duration-500"><button>View More</button></Link>
</div>

<div className="product-cards w-full my-10 h-full">
<Swiper
        // slidesPerView={1}
        breakpoints={breakpoints}
        spaceBetween={30}
        // loop={true}
        // pagination={{
        //   clickable: true,
        // }}
        navigation={true}
        modules={[Pagination, Navigation]}
        className="mySwiper"
      >
 
        {
allProducts.splice(31,40).map(prod=><SwiperSlide key={prod?._id} className="">

<Link to={`/product-details/${prod?.Name}/${prod?._id}`} className="w-[200px] h-[300px] flex flex-col gap-2 bg-white px-2 py-3 shadow hover:shadow-xl">

<div className="w-[150px] h-[120px] mx-auto">
<img src={prod?.Image_Url} alt={prod?.Name} />
</div>

<div className="h-[40px] mt-8">
  <p className="text-xs font-normal text-[#4d4d4f] overflow-hidden">{prod?.Name.slice(0,45)}{prod?.Name.length > 50 ? "..." : ""}</p>
</div>

<div className="mt-3 flex flex-col space-y-2">
<p className="font-bold">R 220</p>
<p className=" inline-flex items-center gap-1 text-sm"><HiStar className='h-4 w-4 text-yellow-400' /> <span>4.3</span><span className='font-medium text-gray-600'>(20)</span> <span><IoIosArrowDown className='h-5 w-5 text-gray-500' /></span> </p>
</div>

</Link>

</SwiperSlide>)
        }

      </Swiper>

</div>

    </div>


    <div className="product-container w-full my-5">
<div className="w-full flex justify-between mb-2 items-center">
<Link to="/brand/books" className="text-base lg:text-xl font-bold text-[#4d4d4f] hover:underline">Must Read Books for 2023</Link>
<Link to="/all/nivea" className="px-3 py-1 border border-[#4d4d4f] text-[#4d4d4f]  font-semibold hover:bg-[#4d4d4f] hover:text-white transition-all duration-500"><button>View More</button></Link>
</div>

<div className="product-cards w-full my-10 h-full">
<Swiper
        // slidesPerView={1}
        breakpoints={breakpoints}
        spaceBetween={30}
        // loop={true}
        // pagination={{
        //   clickable: true,
        // }}
        navigation={true}
        modules={[Pagination, Navigation]}
        className="mySwiper"
      >
 
        {
allProducts.splice(41,50).map(prod=><SwiperSlide key={prod?._id} className="">

<Link to={`/product-details/${prod?.Name}/${prod?._id}`} className="w-[200px] h-[300px] flex flex-col gap-2 bg-white px-2 py-3 shadow hover:shadow-xl">

<div className="w-[150px] h-[120px] mx-auto">
<img src={prod?.Image_Url} alt={prod?.Name} />
</div>

<div className="h-[40px] mt-8">
  <p className="text-xs font-normal text-[#4d4d4f] overflow-hidden">{prod?.Name.slice(0,45)}{prod?.Name.length > 50 ? "..." : ""}</p>
</div>

<div className="mt-3 flex flex-col space-y-2">
<p className="font-bold">R 220</p>
<p className=" inline-flex items-center gap-1 text-sm"><HiStar className='h-4 w-4 text-yellow-400' /> <span>4.3</span><span className='font-medium text-gray-600'>(20)</span> <span><IoIosArrowDown className='h-5 w-5 text-gray-500' /></span> </p>
</div>

</Link>

</SwiperSlide>)
        }

      </Swiper>

</div>

    </div>
</div>

<div className="ad-container hidden lg:flex flex-col gap-4 w-1/3">
<Link to="/brand/" className='w-full bg-white shadow p-2 rounded'>
  <img src="https://tpc.googlesyndication.com/simgad/9356886536132361546?" alt="Add" className='w-full'/>
</Link>

<Link to="/brand/" className='w-full bg-white shadow p-2 rounded'>
  <img src="https://tpc.googlesyndication.com/simgad/1677839733988465028?" alt="Add" className='w-full'/>
</Link>

<Link to="/brand/" className='w-full bg-white shadow p-2 rounded'>
  <img src="https://tpc.googlesyndication.com/simgad/8885274907475278951?" alt="Add" className='w-full'/>
</Link>

<Link to="/brand/" className='w-full bg-white shadow p-2 rounded'>
  <img src="https://tpc.googlesyndication.com/simgad/5496663184552262801?" alt="Add" className='w-full'/>
</Link>

<Link to="/brand/" className='w-full bg-white shadow p-2 rounded'>
  <img src="https://tpc.googlesyndication.com/simgad/8200232379673069226?" alt="Add" className='w-full'/>
</Link>

<Link to="/brand/" className='w-full bg-white shadow p-2 rounded'>
  <img src="https://tpc.googlesyndication.com/simgad/8737224084841716264?" alt="Add" className='w-full'/>
</Link>

<Link to="/brand/" className='w-full bg-white shadow p-2 rounded'>
  <img src="https://tpc.googlesyndication.com/simgad/146114024099689222?" alt="Add" className='w-full'/>
</Link>
</div>
</section>
    );
};

export default Home;